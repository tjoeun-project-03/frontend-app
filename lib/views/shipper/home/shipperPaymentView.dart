...
class ShipperPaymentView extends ConsumerStatefulWidget {
  final double weight;
  final int price; // 최종 결제 금액 (total_cost)
  final int surcharge; // 🚀 추가: 할증료 (total_surcharge_amount)
...
class _ShipperPaymentViewState extends ConsumerState<ShipperPaymentView> {
          ...
          children: [
            ...
            Container(
              ...
              child: Column(children: [
                ...
                _buildSummaryRow(
                    "화물 정보", "${widget.category} / ${widget.weight.toStringAsFixed(1)} 톤"),
                _buildSummaryRow("이동 거리", "${widget.distance.toStringAsFixed(1)} km"),
                const Divider(height: 20),
                // 🚀 요금 상세 내역 추가 (사용자 확인용)
                _buildSummaryRow("기본 운임", "${_formatPrice(widget.price - widget.surcharge)}원"),
                _buildSummaryRow("할증 요금", "+ ${_formatPrice(widget.surcharge)}원"),
              ]),
            ),
            const SizedBox(height: 32),
            const Text("결제 수단",
  ...
  Widget _buildPaymentOption(IconData icon, String label, bool isSelected) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(
            color: isSelected ? const Color(0xFF1A2B88) : Colors.grey[200]!),
  ...
  void _handleOrderSubmit() async {
    if (_nameController.text.isEmpty || _contactController.text.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("수취인 정보를 입력해주세요.")));
      return;
    }

    final String? confirmedInvoiceNo =
    await ref.read(orderViewModelProvider.notifier).getNewInvoiceNo();

    if (confirmedInvoiceNo == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("번호 생성 실패. 다시 시도해주세요.")));
      return;
    }
    ...
    if (result != null && result is Map<String, String?>) {
      _requestFinalOrderCreation(result['paymentKey']!, result['orderId']!,
          int.parse(result['amount']!));
    }
  }

  void _requestFinalOrderCreation(
      String paymentKey, String orderId, int amount) async {
    final success = await ref.read(orderViewModelProvider.notifier).createOrder({
      "paymentKey": paymentKey,
      "invoiceNo": orderId,
      "price": amount, // 최종 금액
      "surcharge": widget.surcharge, // 🚀 추가: 드디어 DB로 할증료가 전달됩니다!
